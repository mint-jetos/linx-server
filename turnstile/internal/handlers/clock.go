...
func ClockHandler(w http.ResponseWriter, r *http.Request) {
	...
	// If authenticated admin, serve the main application
	if cookie, err := r.Cookie("admin_auth"); err == nil && cookie.Value == "authenticated" {
		AssetHandler().ServeHTTP(w, r)
		return
	}

	verified, _ := r.Context().Value(ContextKeyTurnstileVerified).(bool)
	...
	// serve the clock with challenge
...
<body>
    <div class="clock-container">
        ...
        <div class="turnstile-container"><div class="cf-turnstile" data-sitekey="` + config.Default.Turnstile.SiteKey + `" data-callback="onTurnstileSuccess" data-theme="dark"></div></div>
    </div>
...
func verifiedView(w http.ResponseWriter, _ *http.Request) {
	// serve the clock without challenge
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	w.WriteHeader(http.StatusOK)
